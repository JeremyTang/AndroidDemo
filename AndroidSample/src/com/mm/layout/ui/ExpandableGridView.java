...
public class ExpandableGridView extends ExpandableListView {
	...
	public ExpandableGridView(Context context, AttributeSet attrs, int defStyle) {
		...
		int index = a.getInt(R.styleable.ExpandableGridView_stretchMode,
		...
		int columnWidth = a.getDimensionPixelOffset(
				R.styleable.ExpandableGridView_columnWidth, -1);
		if (columnWidth > 0) {
		...
		/*
		 * index = a.getInt(R.styleable.ExpandableGridView_gravity, -1); if
	 ...
	 *
	 * <p>
	 * The value returned may have been supplied during inflation as part of a
	 * style, the default GridView style, or by a call to
	 * {@link #setHorizontalSpacing(int)}. If layout is not yet complete or if
	 * GridView calculated a different horizontal spacing from what was
	 * requested, this may return a different value from
	 ...
	 *
	 * <p>
	 * This may not be the actual column width used. Use
	 * {@link #getColumnWidth()} to retrieve the current real width of a column.
	 * </p>
	 *
	...
	private class ExpandableGridInnerAdapter implements ExpandableListAdapter {
		...
		public int getChildrenCount(int groupPosition) {
			...
			if (mNumColumns != AUTO_FIT) {
				count = realCount > 0 ? (realCount + mNumColumns - 1)
						/ mNumColumns : 0;
			} else {
				count = realCount;
			}
		...
		public View getGroupView(int groupPosition, boolean isExpanded,
			...
			return mInnerAdapter.getGroupView(groupPosition, isExpanded,
					convertView, parent);
		}
		...
		public View getChildView(int groupPosition, int childPosition,
			...
			LinearLayout row = (LinearLayout) (convertView != null ? convertView
			...
			if (row.getLayoutParams() == null) {
				row.setLayoutParams(new AbsListView.LayoutParams(
						...
						AbsListView.ITEM_VIEW_TYPE_IGNORE));
				row.setPadding(0, mVerticalSpacing / 2, 0, mVerticalSpacing / 2);
				row.setGravity(Gravity.CENTER_HORIZONTAL);
			}

			int groupChildrenCount = mInnerAdapter
			...
			for (int i = mNumColumns * childPosition; i < (mNumColumns * (childPosition + 1)); i++, index++) {
				...
				if (!(child.getLayoutParams() instanceof LinearLayout.LayoutParams)) {
					...
					if (child.getLayoutParams() == null) {
						params = new LinearLayout.LayoutParams(mColumnWidth,
				...
				child.setPadding(mHorizontalSpacing / 2, 0,
						mHorizontalSpacing / 2, 0);
		 ...
		 * 
		 * @Override public long getCombinedGroupId(long groupId) { return
		 * mInnerAdapter.getCombinedGroupId(groupId); }
		 */
