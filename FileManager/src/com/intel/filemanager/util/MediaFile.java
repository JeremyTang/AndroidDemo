...
public class MediaFile {
	...
	// int filetype = Integer.parseInt(parser.getAttributeValue(null,
	// "filetype"));
	// int supported = Integer.parseInt(parser.getAttributeValue(null,
	// "supported"));
	//
	...
	private static void loadFileTypesStatic() {
		Log.w(TAG,
				"load file types from XML failed! Try to add basic file types.");
		...
		addFileType("M3U8", FILE_TYPE_HTTPLIVE,
		...
		// Borqs: drm support
		addFileType("DCF", FILE_TYPE_OMADRM_V1,
				"application/vnd.oma.drm.content", 1);
		addFileType("DM", FILE_TYPE_OMADRM_V1,
				"application/vnd.oma.drm.message", 1);
	...
	public static boolean isMediaFile(String path) {
		return isAudioFileType(path) || isVideoFileType(path)
	...
	public static String getRealMimeType(Context ctx, String path) {
		...
		Uri uri = path.startsWith(Environment.getExternalStorageDirectory()
				.getAbsolutePath()) ? MediaStore.Video.Media.EXTERNAL_CONTENT_URI
		...
		try {
			...
			if (null == c || 0 == c.getCount())
				Log.d(TAG,
						"###### Item is not in 'Video' table, will lookup it in 'Audio' table");
			else {
				c.moveToFirst();
				String mimeType = c
						.getString(c
		...
		uri = path.startsWith(Environment.getExternalStorageDirectory()
				.getAbsolutePath()) ? MediaStore.Audio.Media.EXTERNAL_CONTENT_URI
		...
		try {
			...
			if (null == c || 0 == c.getCount()) {
				...
				// there is no such item in the audio_meta table, try lookup it
				// in static mime table
				return getMimeType(path);
			} else {
				c.moveToFirst();
				String mimeType = c
	...
	private static int getIconForFileType(int fileType) {
		...
		if (fileType == MediaFile.FILE_TYPE_JAD
				|| fileType == MediaFile.FILE_TYPE_JAR
		...
		if (fileType == MediaFile.FILE_TYPE_ICS
				|| fileType == MediaFile.FILE_TYPE_VCS) {
