...
public class FileMainActivity extends Activity implements TabListener {
		...
		if (FileUtil.getMountedSDCardCount(mActivity) == 1) {
			if (CURRENTPATH.startsWith(FileUtil.PATH_SDCARD_2)
				...
				CURRENTPATH = Environment.getExternalStorageDirectory()
						.getAbsolutePath();
			} else if (CURRENTPATH.startsWith(pathSDCard)
					&& !FileUtil.isMounted(mActivity, pathSDCard)) {
	...
	protected BroadcastReceiver mReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			Log.v(TAG,
					"SubCategoryActivity receive intent:" + intent.toString());
			...
			if (action.equals(Intent.ACTION_MEDIA_MOUNTED)) {
				...
				mFileDir.initSwitchButton(mFileDir.getView());
				mFileDir.setActionBarTitle(getCurrentPath());
				mFileDir.CURRENTPATH = Environment
	...
	private void handleIntent(Intent intent) {
		...
		if (!TextUtils.isEmpty(action)) {
			if (action.equals(Intent.ACTION_VIEW)) {
				...
				mFileDir.mLaunchedWithPath = false;
				mActionBar.selectTab(getTab(TAB_INDEX_DIRECTORY));
				intent.putExtra(FileMainActivity.EXTRA_FILE_PATH,
	...
	private class PageChangeListener extends SimpleOnPageChangeListener {
		...
		public void onPageSelected(int position) {
			...
			if (position == TAB_INDEX_CATEGORY && mFragmentCategory != null) {
				((FileCategoryFragment) mFragmentCategory)
						.refreshCategoryInfo();
			}
		}
	}
